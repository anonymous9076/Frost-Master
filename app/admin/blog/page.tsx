...
import { deleteBlog, getAllBlogs } from "@/app/api/Blog";
interface Article {
  _id: string;
  title: string;
  imagePath?: string;
...
const BlogCreation = () => {
  ...
  const [blogsData, setBlogsData] = useState<Article[]>([]);
                ...
                <div className=" h-fit max-h-[60dvh] overflow-y-auto w-full  pt-[2rem]">
                  ...
                  {blogsData ? (
                    blogsData?.map((data, index) => (
                      <div key={index}>
                        <ul className=" my-1 p-0 flex items-center py-3 px-[1.5rem] bg-white rounded-sm shadow-sm  ">
